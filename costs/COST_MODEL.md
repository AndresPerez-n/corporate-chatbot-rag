...
5. **Batch ingestion off-hours** — no direct per-token saving (embeddings are local), but avoids API rate-limit throttling if switching to API-based embeddings later.
